...
구문상의 편의를 위해 Move의 일부 함수는 값에 대한 "메서드"로 호출할 수 있습니다. 이는 `.` 연산자를 사용하여 함수를 호출함으로써 이루어지며, 여기서 `.`의 왼쪽에 있는 값이 함수의 첫 번째 인자(때때로 수신자라고 함)가 됩니다. 해당 값의 타입이 호출되는 함수를 정적으로 결정합니다. 이는 다른 언어와 중요한 차이점인데, 다른 언어에서는 이 구문이 런타임에 호출할 함수가 결정되는 동적 호출을 나타낼 수 있습니다. Move에서는 모든 함수 호출이 정적으로 결정됩니다.

간단히 말해, 이 구문은 `use`로 별칭을 만들지 않고도 함수를 더 쉽게 호출할 수 있고, 함수의 첫 번째 인자를 명시적으로 차용하지 않아도 되도록 하기 위해 존재합니다. 또한 함수를 호출하는 데 필요한 보일러플레이트를 줄이고 함수 호출을 연결하기 쉽게 만들어 코드를 더 읽기 쉽게 만들 수 있습니다.
