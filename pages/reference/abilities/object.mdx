...
An object is a [`struct`](../structs) with the [`key`](../abilities#key) ability. The first
field of the struct must be `id: sui::object::UID`. This 32-byte field (a strongly typed wrapper
around an [`address`](../primitive-types/address)) is then used to uniquely identify the object.
